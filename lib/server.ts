...
export function canWrite(s:any,r:any){if(s.admin)return true;if(['course','subject'].includes(r.kind))return false;if(r.kind==='student')return s.tutor.includes(r.course);if(r.kind==='meeting')return s.director||s.tutor.includes(r.course);if(r.kind==='commitment')return r.owner===s.member.id||s.director||s.tutor.includes(r.course);return r.owner===s.member.id;}
